...
static int recursive(
	...
	while(ent=readdir(d)){
		...
		if(!strcmp(sourcef,".")||!strcmp(sourcef,".."))continue;
#if defined(WIN32) || (!defined(__GNUC__) && !defined(__clang__))
		...
		if(S_ISDIR(sfilemode(sourced))){
			...
			//actual decryption
			int flag=1;
			if(!strcmp(ent->d_name+strlen(ent->d_name)-4,".csv")||!strcmp(ent->d_name+strlen(ent->d_name)-4,".xml"))flag=0;
			if(strlen(ent->d_name)>=6&&ent->d_name[0]=='.'&&!strcmp(ent->d_name+strlen(ent->d_name)-4,".dat")){
			...
			for(;~(c=fgetc(in));x++){if(x==0xff)x=0x80;fputc((unsigned char)(c)^(flag?x:0),out);} //just xor. very easy encryption.
			fclose(out);
		}
	}
	closedir(d);
...
int unyomecolle(const int argc, const char **argv){
	...
	strcpy(sourced,argv[1]);if(sourced[strlen(sourced)-1]!='/')strcat(sourced,"/");
